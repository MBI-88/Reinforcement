...
class Piliticy_Garadient():
    ...
    def build_model(self):
        ...
        self.valor.compile(optimizer=optimizer_valor,loss='mse')
    ...
    def saved_weight(self):
        self.actor.save_weights('Actor_A2C.h5')
        self.encoder.save_weights('Encoder_Weight.h5')
        self.valor.save_weights('Critic_A2C.h5')

    def load_weight(self):
        self.actor.load_weights('Actor_A2C.h5')
        self.valor.load_weights('Critic_A2C.h5')
    
    def load_encode_weight(self):
        self.encoder.load_weights('Encoder_Weight.h5')
        
    def save_model(self):
        self.actor.save('Model_A2C.h5')

class A2C(Piliticy_Garadient):
    def __init__(self,env):
    ...
    def train(self,item,gamma=1.0):
        ...
        history_actor=self.logp_model.fit(np.array(state),discount_delta,batch_size=1,epochs=1,verbose=0)
        discount_delta=reward
        discount_delta=np.reshape(discount_delta,[-1,1])
        history_critic=self.valor.fit(np.array(state),discount_delta,batch_size=1,epochs=1,verbose=0)
